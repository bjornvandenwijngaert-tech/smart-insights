...
var state = {
  ...
  hostVia:          "",    // where server/dbName came from, for diagnostics
  editMode:         false,
...
// The Activity Report deliberately does NOT use this — it reports raw stop time.
var OVERNIGHT_MIN_HOURS = 3;

// Local calendar day. Everything these reports display is local time, so the day
...
// Stop minutes for the Legacy Trip History report, or null when the stop is an
// overnight park or unmeasurable. Falls back to the raw field if the trip
// predates the map. Activity Report call sites read stopDuration directly.
...
// three copies in sync (here, legacy-trip-report/src/app.js, and that repo): the
// URL shape below is not documented anywhere reliable.
//
...
//
// Timezone note: the Activity Report add-in builds these from the MyGeotab profile
...
// already the real MyGeotab URL: https://my.geotab.com/<database>/#smartInsights
  ...
  output.innerHTML = vids.map(function (vid) {
    ...
    var dayHtml = dayBlocks.map(function (block) {
      ...
      return "<div class='dd-table-wrap' style='margin-bottom:12px'><table class='dd-table'>" +
        ...
        "<tr style='font-weight:700;background:var(--surface-2)'><td>" + block.date + " Total</td><td>" + fmtDist(dayDistM, 2) + " in " + fmtDurWhole(dayDrive) + "</td><td></td><td>" + dayCount.length + " stops</td><td>" + fmtDurWhole(dayIdle) + "</td><td>" + fmtDurWhole(dayStop) + "</td><td></td></tr>" +
        "</tbody></table></div>";
    }).join("");
